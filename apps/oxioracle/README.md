...
I created OXIOracle as a feature branch inside of [Atocha](https://github.com/johnnycopes/atocha/tree/oxio), my personal Nx monorepo where I manage all of my current applications and libraries. I chose to add it to my monorepo rather than create an independent repository for this project in order to reuse existing code, utilize my CI/CD pipeline, and save significant time when installing libraries and writing tests.

### Data

I used the [JSONplaceholder API](https://jsonplaceholder.typicode.com) mentioned in the project instructions to supply the application with data. I call its `/todos` endpoint, which always returns a consistent 200 [todos](https://jsonplaceholder.typicode.com/todos). Exactly one API call is made in OXIOracle to fetch the data (while the application is booting, inside the [`APP_INITIALIZER` function](https://github.com/johnnycopes/atocha/blob/8d757a3da145317212fd6f8d76004a02ae269942/apps/oxioracle/src/app/app.config.ts#L33-L37)) and the results are then stored in local state for consumption by the UI.

All state for fetching todo data and managing local todos state happens in the [`TodoService`](https://github.com/johnnycopes/atocha/blob/oxio/libs/oxioracle/data-access/src/lib/todo.service.ts). Internally, that service uses the [`ApiService`](https://github.com/johnnycopes/atocha/blob/oxio/libs/oxioracle/data-access/src/lib/api.service.ts) to fetch the data (with help from Angular's `HttpClient` service) and my own custom [`State` class](https://github.com/johnnycopes/atocha/blob/main/libs/core/data-access/src/lib/state.ts) (a generic RxJS observable store) for state management.

All chart is managed by the [`ChartService`](https://github.com/johnnycopes/atocha/blob/oxio/libs/oxioracle/feature-data/src/lib/charts/charts.service.ts). However, nothing about the charts' state is persisted, so its purpose is ultimately to transform the todo data into the correct shape for the charts to consume.

### Sortable table

Creating a data grid (aka table) with meaningful functionality is an onerous undertaking in UI development. Due to the limited scope of this project, I chose to use [Google's Angular Material `<mat-table>`](https://material.angular.io/components/table/overview) for this feature. The todo data fields are separated by columns, which can each be sorted in ascending or descending order. The current sorting configuration is stored in local state.
...
I leaned on Angular's `ReactiveFormsModule` to build the form for adding a new todo. The ID is generated by the application, but the user ID, title, and completion state are defined by the user. There's validation logic on all fields that will throw visible errors if everything isn't filled out on an attempted submission. There's also a toast message that launches when a new todo is successfully submitted.

### Routing

I used Angular's routing capabilities to define the application routes. The table and charts each live in their own pages. I opted to put the form on its own page as well, since the router was already established and it allowed for the other features to maximize their space.

The application defaults to the `/table` route on load if a specific route isn't defined. There's also an error page that renders if the user attempts to visit an invalid route. All legal pages can be navigated to via the header at the top of the application.
...
2. The [`Form` class](https://github.com/johnnycopes/atocha/blob/oxio/libs/oxioracle/feature-data/src/lib/form/form.spec.ts)
...
1. Add error handling in case the HTTP call fails
1. Add more capabilities to the table, like searching
1. Create more charts
1. Improve the uniformity of the UI (make the table and charts look more integrated into the app)
1. Come up with a more sophisticated way to integrate the form into the application
