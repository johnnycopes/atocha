import type { DifficultyOption } from '../app/game/difficulty';
...
describe('getDifficulty', () => {
  it('returns static difficulty value', () => {
    ...
    expect(
      getDifficulty(mockItem.difficulty, ['Branch & Claw', 'Jagged Earth'])
    ).toBe(4);
    expect(getDifficulty(mockItem.difficulty, ['Promo Pack 1'])).toBe(4);
    expect(getDifficulty(mockItem.difficulty, ['Promo Pack 2'])).toBe(4);
    expect(
      getDifficulty(mockItem.difficulty, ['Promo Pack 1', 'Promo Pack 2'])
    ).toBe(4);
    expect(
      getDifficulty(mockItem.difficulty, [
        'Branch & Claw',
        'Jagged Earth',
        'Promo Pack 1',
        'Promo Pack 2',
