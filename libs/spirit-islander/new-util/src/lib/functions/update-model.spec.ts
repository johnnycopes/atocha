...
describe('updateModel', () => {
  it("adds individual expansion's items to existing model", () => {
    expect(
      updateModel(createSpiritsModel, [], ['Branch & Claw'], 'Branch & Claw')
    ).toStrictEqual([
  ...
  it("removes all expansions' items from existing model", () => {
    expect(updateModel(createSpiritsModel, [], [], 'Expansions')).toStrictEqual(
      []
    );

    expect(
      updateModel(createSpiritsModel, ['Thunderspeaker'], [], 'Expansions')
    ).toStrictEqual(['Thunderspeaker']);
